...
*   **Secrets:** ในโฟลเดอร์ secret ให้กำหนดค่าในไฟล์ `.db_root_pass.txt` และ `.db_user_pass.txt` เพื่อเก็บรหัสผ่าน และเรียกใช้ผ่าน `secrets` (ห้ามพิมพ์รหัสผ่านลงใน YAML)
...
*   สร้างไฟล์ `.env` เก็บค่าตัวแปร: `itthchai`, `HTTP_PORT`, `PMA_PORT`, `DB_NAME` และนำไปใช้ใน `compose.yaml` ทั้งหมด
...
('1660705268', 'Itthichai Faengchaiyaphum', 'itthichai-dev', 'itthichai.faen@bumail.net', 'Submitted');
...
1.  **Files:** ส่งทุกไฟล์
    *   ไฟล์ `compose.yaml`
    *   ไฟล์ `.env`
    *   ไฟล์ความลับอื่นๆ (`.db_root_pass.txt`, `.db_user_pass.txt`)
...
#   I T - 4 6 4 - 3 2 8 B - i t t h i c h a i - A s s i g n m e n t 0 7  
 